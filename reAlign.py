...
def changeNames():
    ...
    for number in numbers:
        nameList = []
        newLine = []
        with open("reformatFolder/Pillar"+str(number)+"_gapsClean.fas","r")as old:
            dictionary = {}
            f=old.readlines()
            for line in f:
                if ">"  in line:
            ...
            for i in range(0,len(nameList)):
                for j in range(i+1, len(nameList)):
                    if nameList[i][1:8] == nameList[j][1:8]:
                        if int(nameList[i][8:]) < int(nameList[j][8:]):
                            dic = {nameList[i]:"01", nameList[j]:"02"}
                        else:
                            dic = {nameList[j]:"01", nameList[i]:"02"}
                        dictionary.update(dic)
                if nameList[i] not in list(dictionary.keys()):
                    dictionary.update({nameList[i]:"01"})
            for line in f:
                if ">" in line:
                    line = line[0:8] + dictionary[line] +"\n"
                newLine.append(line)
            with open("Alignment/Pillar"+number+"R.fasta","w") as new:
                for i in newLine:
                    new.write(i)
