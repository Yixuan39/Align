...
{
	int i,j, namepos;
	char dump, line[202], new_name[700];
	Sequence_dataset *the_data;
	...
	var_stream->getline(line, 201);
      
	while (word_match(line, 201, "Begin Data", 10)!=1)
		var_stream->getline(line,201);
      
      
	while (word_match(line, 201, "dimensions", 10)!=1)
		var_stream->getline(line,201);
      
      
	i=loc_word_match(line, 201, "ntax", 4)+5;
    num_taxa=string_to_int(&line[i]);
      
    ntaxa=num_taxa;
      
      
    i=loc_word_match(line, 201, "nchar", 5)+6;
    num_sites=string_to_int(&line[i]);
      
    size=num_sites;
    
    the_data = new Sequence_dataset(num_taxa, size, ctype);
      
    var_stream->getline(line,82);
      
	while (word_match(line, 201, "Matrix", 6)!=1)
		var_stream->getline(line,201); 
