...
{
    int i, pre_dec, exp_size=0, neg=0, less_than_one, pos=0;
    double temp=1;
    ...
    if (long_enough==TRUE) {
          if (is_zero==TRUE)
              ...
              if (exp_form==FALSE) {
                  ...
                  pre_dec=(int)log10(temp)+1;
                  the_string[pos]=(((int)temp)/((int)pow(10,pre_dec-1)))+48;
                  the_string[pos]='.';
                  pos++;
                  temp-=(((int)temp)/((int)pow(10,pre_dec-1)))*((int)pow(10,pre_dec-1));
                  
                  for (i=0; i<precision; i++) {
                      the_string[pos]=(((int)temp)/((int)pow(10,i)))+48;
                      temp-=(((int)temp)/((int)pow(10,i)))*((int)pow(10,i));
                      pos++;
                  }
